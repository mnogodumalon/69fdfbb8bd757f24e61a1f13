...
import { GastViewDialog } from '@/components/dialogs/GastViewDialog';
import { BuchungDialog } from '@/components/dialogs/BuchungDialog';
import { BuchungViewDialog } from '@/components/dialogs/BuchungViewDialog';
...
const GAST_FIELDS = [
  ...
  { key: 'telefon', label: 'Telefon', type: 'string/text' },
];
const BUCHUNG_FIELDS = [
  { key: 'notizen', label: 'Notizen', type: 'string/textarea' },
  { key: 'abreise', label: 'Abreise', type: 'date/date' },
  { key: 'naechte', label: 'Nächte', type: 'number' },
  { key: 'einzelpreis_pro_nacht', label: 'Einzelpreis pro Nacht', type: 'number' },
  { key: 'anreise', label: 'Anreise', type: 'date/date' },
  { key: 'gesamtpreis', label: 'Gesamtpreis', type: 'number' },
  { key: 'gast', label: 'Gast', type: 'applookup/select', targetEntity: 'gast', targetAppId: 'GAST', displayField: 'vorname' },
  { key: 'buchungsnummer', label: 'Buchungsnummer', type: 'string/text' },
  { key: 'status', label: 'Status', type: 'lookup/radio', options: [{ key: 'storniert', label: 'Storniert' }, { key: 'bestaetigt', label: 'Bestätigt' }, { key: 'offen', label: 'Offen' }] },
  { key: 'mwst', label: 'MwSt. (19 %)', type: 'number' },
  { key: 'brutto', label: 'Brutto', type: 'number' },
];

const ENTITY_TABS = [
  { key: 'gast', label: 'Gast', pascal: 'Gast' },
  { key: 'buchung', label: 'Buchung', pascal: 'Buchung' },
] as const;
...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<EntityKey>('gast');
  ...
  const getRecords = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gast': return (data as any).gast as Gast[] ?? [];
      case 'buchung': return (data as any).buchung as Buchung[] ?? [];
      default: return [];
    }
  }, [data]);
  ...
  const getLookupLists = useCallback((entity: EntityKey) => {
    ...
    switch (entity) {
      case 'buchung':
        lists.gastList = (data as any).gast ?? [];
        break;
    }
    return lists;
  }, [data]);
  ...
  const getApplookupDisplay = useCallback((entity: EntityKey, fieldKey: string, url?: unknown) => {
    ...
    if (entity === 'buchung' && fieldKey === 'gast') {
      const match = (lists.gastList ?? []).find((r: any) => r.record_id === id);
  ...
  const getFieldMeta = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gast': return GAST_FIELDS;
      case 'buchung': return BUCHUNG_FIELDS;
      default: return [];
    }
  }, []);
  ...
  const getServiceMethods = useCallback((entity: EntityKey) => {
    switch (entity) {
      case 'gast': return {
        create: (fields: any) => LivingAppsService.createGastEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateGastEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteGastEntry(id),
      };
      case 'buchung': return {
        create: (fields: any) => LivingAppsService.createBuchungEntry(fields),
        update: (id: string, fields: any) => LivingAppsService.updateBuchungEntry(id, fields),
        remove: (id: string) => LivingAppsService.deleteBuchungEntry(id),
      };
      default: return null;
    }
  }, []);
  ...
  return (
    ...
    >
      ...
      {(createEntity === 'gast' || dialogState?.entity === 'gast') && (
        <GastDialog
          open={createEntity === 'gast' || dialogState?.entity === 'gast'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'gast' ? handleUpdate : (fields: any) => handleCreate('gast', fields)}
          defaultValues={dialogState?.entity === 'gast' ? dialogState.record?.fields : undefined}
          enablePhotoScan={AI_PHOTO_SCAN['Gast']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Gast']}
        />
      )}
      {(createEntity === 'buchung' || dialogState?.entity === 'buchung') && (
        <BuchungDialog
          open={createEntity === 'buchung' || dialogState?.entity === 'buchung'}
          onClose={() => { setCreateEntity(null); setDialogState(null); }}
          onSubmit={dialogState?.entity === 'buchung' ? handleUpdate : (fields: any) => handleCreate('buchung', fields)}
          defaultValues={dialogState?.entity === 'buchung' ? dialogState.record?.fields : undefined}
          gastList={(data as any).gast ?? []}
          enablePhotoScan={AI_PHOTO_SCAN['Buchung']}
          enablePhotoLocation={AI_PHOTO_LOCATION['Buchung']}
        />
      )}
      {viewState?.entity === 'gast' && (
        <GastViewDialog
          open={viewState?.entity === 'gast'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'gast', record: r }); }}
        />
      )}
      {viewState?.entity === 'buchung' && (
        <BuchungViewDialog
          open={viewState?.entity === 'buchung'}
          onClose={() => setViewState(null)}
          record={viewState?.record}
          onEdit={(r: any) => { setViewState(null); setDialogState({ entity: 'buchung', record: r }); }}
          gastList={(data as any).gast ?? []}
        />
      )}
