...
import type { Gast } from '@/types/app';
...
export default function GastPage() {
  const [records, setRecords] = useState<Gast[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Gast | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Gast | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Gast | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getGast());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Gast['fields']) {
    await LivingAppsService.createGastEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Gast['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateGastEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteGastEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Gast"
      subtitle={`${records.length} Gast im System`}
      action={
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={5} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Gast. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
