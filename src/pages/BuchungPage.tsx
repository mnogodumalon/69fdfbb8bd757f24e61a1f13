...
export default function BuchungPage() {
  const [records, setRecords] = useState<Buchung[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Buchung | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Buchung | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Buchung | null>(null);
  const [sortKey, setSortKey] = useState('');
  const [sortDir, setSortDir] = useState<'asc' | 'desc'>('asc');
  const [gastList, setGastList] = useState<Gast[]>([]);
  ...
  async function loadData() {
    setLoading(true);
    try {
      const [mainData, gastData] = await Promise.all([
  ...
  async function handleCreate(fields: Buchung['fields']) {
    await LivingAppsService.createBuchungEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Buchung['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateBuchungEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteBuchungEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Buchung"
      subtitle={`${records.length} Buchung im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('naechte')}>
                <span className="inline-flex items-center gap-1">
                  Nächte
                  {sortKey === 'naechte' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('einzelpreis_pro_nacht')}>
                <span className="inline-flex items-center gap-1">
                  Einzelpreis pro Nacht
                  {sortKey === 'einzelpreis_pro_nacht' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('gast')}>
                <span className="inline-flex items-center gap-1">
                  Gast
                  {sortKey === 'gast' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('mwst')}>
                <span className="inline-flex items-center gap-1">
                  MwSt. (19 %)
                  {sortKey === 'mwst' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('brutto')}>
                <span className="inline-flex items-center gap-1">
                  Brutto
                  {sortKey === 'brutto' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{record.fields.naechte ?? '—'}</TableCell>
                <TableCell>{record.fields.einzelpreis_pro_nacht ?? '—'}</TableCell>
                ...
                <TableCell><span className="inline-flex items-center bg-secondary border border-[#bfdbfe] text-[#2563eb] rounded-[10px] px-2 py-1 text-sm font-medium">{getGastDisplayName(record.fields.gast)}</span></TableCell>
                ...
                <TableCell>{record.fields.mwst ?? '—'}</TableCell>
                <TableCell>{record.fields.brutto ?? '—'}</TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Buchung. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
