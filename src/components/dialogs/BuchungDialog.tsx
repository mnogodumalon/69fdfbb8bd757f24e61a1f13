...
interface BuchungDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Buchung['fields']) => Promise<void>;
  defaultValues?: Buchung['fields'];
  gastList: Gast[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function BuchungDialog({ open, onClose, onSubmit, defaultValues, gastList, enablePhotoScan = true, enablePhotoLocation = true }: BuchungDialogProps) {
  const [fields, setFields] = useState<Partial<Buchung['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'buchung');
      await onSubmit(clean as Buchung['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="gast" entity="Gast">\n${JSON.stringify(gastList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "notizen": string | null, // Notizen\n  "abreise": string | null, // YYYY-MM-DD\n  "naechte": number | null, // Nächte\n  "einzelpreis_pro_nacht": number | null, // Einzelpreis pro Nacht\n  "anreise": string | null, // YYYY-MM-DD\n  "gesamtpreis": number | null, // Gesamtpreis\n  "gast": string | null, // Display name from Gast (see <available-records>)\n  "buchungsnummer": string | null, // Buchungsnummer\n  "status": LookupValue | null, // Status (select one key: "storniert" | "bestaetigt" | "offen") mapping: storniert=Storniert, bestaetigt=Bestätigt, offen=Offen\n  "mwst": number | null, // MwSt. (19 %)\n  "brutto": number | null, // Brutto\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["gast"]);
        ...
        if (gastName) {
          const gastMatch = gastList.find(r => matchName(gastName!, [[r.fields.vorname ?? '', r.fields.nachname ?? ''].filter(Boolean).join(' ')]));
          if (gastMatch) merged['gast'] = createRecordUrl(APP_IDS.GAST, gastMatch.record_id);
        }
        return merged as Partial<Buchung['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Buchung bearbeiten' : 'Buchung hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <Select
              value={extractRecordId(fields.gast) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, gast: v === 'none' ? undefined : createRecordUrl(APP_IDS.GAST, v) }))}
            >
              <SelectTrigger id="gast"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
