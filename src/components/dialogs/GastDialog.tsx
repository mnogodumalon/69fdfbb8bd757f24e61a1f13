...
interface GastDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Gast['fields']) => Promise<void>;
  defaultValues?: Gast['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function GastDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: GastDialogProps) {
  const [fields, setFields] = useState<Partial<Gast['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'gast');
      await onSubmit(clean as Gast['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      setFields(prev => {
        ...
        return merged as Partial<Gast['fields']>;
      });
      setAiText('');
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Gast bearbeiten' : 'Gast hinzufügen';
