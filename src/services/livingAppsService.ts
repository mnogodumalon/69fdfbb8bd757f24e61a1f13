...
import type { Gast, Buchung, CreateGast, CreateBuchung } from '@/types/app';
...
export class LivingAppsService {
  // --- GAST ---
  static async getGast(): Promise<Gast[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GAST}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Gast[];
    return enrichLookupFields(records, 'gast');
  }
  static async getGastEntry(id: string): Promise<Gast | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GAST}/records/${id}`);
    const record = { record_id: data.id, ...data } as Gast;
    return enrichLookupFields([record], 'gast')[0];
  }
  static async createGastEntry(fields: CreateGast) {
    return callApi('POST', `/apps/${APP_IDS.GAST}/records`, { fields: cleanFieldsForApi(fields as any, 'gast') });
  }
  static async updateGastEntry(id: string, fields: Partial<CreateGast>) {
    return callApi('PATCH', `/apps/${APP_IDS.GAST}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gast') });
  }
  static async deleteGastEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GAST}/records/${id}`);
  }

  // --- BUCHUNG ---
  static async getBuchung(): Promise<Buchung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.BUCHUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Buchung[];
    return enrichLookupFields(records, 'buchung');
  }
  static async getBuchungEntry(id: string): Promise<Buchung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.BUCHUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Buchung;
    return enrichLookupFields([record], 'buchung')[0];
  }
  static async createBuchungEntry(fields: CreateBuchung) {
    return callApi('POST', `/apps/${APP_IDS.BUCHUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'buchung') });
  }
  static async updateBuchungEntry(id: string, fields: Partial<CreateBuchung>) {
    return callApi('PATCH', `/apps/${APP_IDS.BUCHUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'buchung') });
  }
  static async deleteBuchungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.BUCHUNG}/records/${id}`);
  }
